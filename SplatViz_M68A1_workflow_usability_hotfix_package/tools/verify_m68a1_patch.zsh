...
print "Frame-safe layout + navigation smoke"
layout_smoke_json=$(/usr/bin/mktemp /tmp/m68a1_layout_smoke.XXXXXX)
if ! M68A1_LAYOUT_SMOKE_JSON="${layout_smoke_json}" \
	"${GODOT}" --headless --path "${SPLATVIZ_PROJECT}" -s "${PACKAGE_ROOT}/tools/m68a1_frame_safe_layout_smoke.gd" >/dev/null
then
...
assert data["startup_mode"] == "Camera POV" or data["startup_mode"] == "Rigging / Lighting"
...
assert data["sample_output_root"].endswith("render_selected_smoke_" + data["timestamp"])
...
report_smoke_root=$(/usr/bin/mktemp -d /tmp/m68a1_report_smoke.XXXXXX)
report_smoke_json=$(/usr/bin/mktemp /tmp/m68a1_report_payload.XXXXXX)
if ! M68A1_REPORT_SMOKE_ROOT="${report_smoke_root}" \
	M68A1_REPORT_SMOKE_JSON="${report_smoke_json}" \
	SPLATVIZ_LAYOUT="Frame-Safe 36-Camera Multi-Tier" \
	"${GODOT}" --headless --path "${SPLATVIZ_PROJECT}" -s "${PACKAGE_ROOT}/tools/m68a1_report_smoke.gd" >/dev/null
then
...
viewer_smoke_json=$(/usr/bin/mktemp /tmp/m68a1_splat_viewer.XXXXXX)
viewer_smoke_root=$(/usr/bin/mktemp -d /tmp/m68a1_splat_viewer.XXXXXX)
if ! M68A1_SPLAT_VIEWER_SMOKE_JSON="${viewer_smoke_json}" \
	M68A1_SPLAT_VIEWER_SMOKE_ROOT="${viewer_smoke_root}" \
	"${GODOT}" --headless --path "${SPLATVIZ_PROJECT}" -s "${PACKAGE_ROOT}/tools/m68a1_splat_viewer_smoke.gd" >/dev/null
then
	failures+=("Splat viewer smoke script failed")
fi
if ! /usr/bin/python3 - "${viewer_smoke_json}" <<'PY'
import json, sys
data = json.load(open(sys.argv[1]))
...
print "M68A1 verification passed."
