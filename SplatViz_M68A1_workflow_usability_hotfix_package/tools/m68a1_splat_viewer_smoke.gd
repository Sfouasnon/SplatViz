...
func _run() -> void:
	var smoke_root := OS.get_environment("M68A1_SPLAT_VIEWER_SMOKE_ROOT")
	if smoke_root == "":
		smoke_root = "/tmp/m68a1_splat_viewer_smoke"
	DirAccess.make_dir_recursive_absolute(smoke_root)
	var ascii_path := smoke_root.path_join("sample_ascii.ply")
	var binary_path := smoke_root.path_join("sample_binary.ply")
	...
	var out_path := OS.get_environment("M68A1_SPLAT_VIEWER_SMOKE_JSON")
	if out_path != "":
