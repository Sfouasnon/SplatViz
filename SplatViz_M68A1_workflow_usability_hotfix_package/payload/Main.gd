...
func _build_height_scale() -> void:
	...
	var origin = Vector3(0.95, 0.0, 0.85)
	...
	for i in range(11):
		...
		if i % 2 == 0:
			...
			label.position = origin + Vector3(0.28, meters, 0.0)
			label.font_size = 22
	...
	title.position = origin + Vector3(0.0, ROBOT_HEIGHT_M + 0.22, 0.0)
	title.font_size = 26
...
func _build_ui() -> void:
	...
	_add_toolbar_label(th, "SplatViz M6.7G2", 22, Color(0.68, 1.0, 0.82), 165)
...
" + export_root_path, 12, Color(0.70,0.84,0.78))
	...
	stills_folder_dialog.title = "Choose Still Image Folder"
...
func _build_camera_pov_preview(root: Control, screen_size: Vector2) -> void:
	...
	camera_pov_preview_panel.add_theme_stylebox_override("panel", _panel_style(Color(0.0, 0.010, 0.012, 0.92)))
...
func _m67b_build_splat_view_tools(root: Control, screen_size: Vector2) -> void:
	...
	m67b_splat_tools_panel.size = Vector2(292, 230)
...
func _m67b_render_selected_camera_size(size: Vector2i) -> void:
	...
	var out_root: String = _m68a_make_timestamped_output_root("splatviz_clean_m67b_" + tag + "_selected", export_timestamp)
...
func _m67b_render_all_cameras_size(size: Vector2i) -> void:
	var tag: String = _m67b_render_tag(size)
	var export_timestamp := _m68a_timestamp()
	var out_root: String = _m68a_make_timestamped_output_root("splatviz_clean_m67b_" + tag + "_all", export_timestamp)
...
func _m67b_layout_report_html(entries: Array, stamp: String) -> String:
	...
	html += "<section class='page'><h1>SplatViz M67B Camera Layout Report</h1><p>Created " + stamp + ". Layout: " + layout_name + ". Stage: %.2fm × %.2fm.</p>" % [STAGE_W_M, STAGE_D_M]
...
func _update_inspector() -> void:
	...
	if mode == "Focus":
		...
		text += "The centered 16:9 Camera POV preview uses the same camera transform, FOV, roll, and clean-view hiding as exported stills. If it differs from a PNG, treat that as a parity bug.\n\n"
		text += _m66d_camera_capture_summary(c, CLEAN_RENDER_SIZE) + "\n\n"
		text += "Quick navigation\nPrevious / Next buttons wrap camera order. Optional shortcuts: [ previous, ] next.\n\n"
		...
		text += "Interpretation\nDebug point preview — not final anisotropic 3DGS rasterization. Use this view for workflow validation only, not quality judgment. M6.5A filters invalid/NaN vertices from Msplat PLYs and auto-fits tiny COLMAP-scale results into the performer envelope for visual inspection.\n\n"
...
func _render_source_selected_camera() -> void:
	...
	var c: Dictionary = cameras[selected_index]
	var export_timestamp := _m68a_timestamp()
	var out_root: String = _m68a_make_timestamped_output_root("splatviz_source_m66e_selected", export_timestamp)
	var out_dir: String = out_root + "/images/" + str(c["id"])
	DirAccess.make_dir_recursive_absolute(out_dir)
	var path: String = out_dir + "/" + _camera_unique_filename(c)
...
func _render_source_all_cameras() -> void:
	var export_timestamp := _m68a_timestamp()
	var out_root: String = _m68a_make_timestamped_output_root("splatviz_source_m66e_all", export_timestamp)
...
func _render_selected_camera() -> void:
	...
	var c = cameras[selected_index]
	var export_timestamp := _m68a_timestamp()
	var out_root = _m68a_make_timestamped_output_root("splatviz_clean_m63_selected", export_timestamp)
...
func _render_all_cameras() -> void:
	var export_timestamp := _m68a_timestamp()
	var out_root = _m68a_make_timestamped_output_root("splatviz_clean_m63_all", export_timestamp)
...
func _debug_dump_msplat_clean_render_state(dataset_root: String, tag: String) -> void:
	...
	var lines: Array = []
	lines.append("SplatViz M6.7G2 clean render state debug: " + tag)
	...
	var f = FileAccess.open(dataset_root + "/splatviz_visible_mesh_dump_m61.txt", FileAccess.WRITE)
	if f != null:
		f.store_string("\n".join(lines) + "\n")
		f.close()
...
func _msplat_input_path() -> String:
	var dataset = msplat_dataset_root if msplat_dataset_root != "" else export_root_path + "/splatviz_msplat_dataset_m63"
...
func _msplat_command_string() -> String:
	...
	var result = msplat_result_root if msplat_result_root != "" else export_root_path + "/splatviz_msplat_result_m63"
	var ply = result + "/splatviz_m65a_TIMESTAMP_msplat_" + str(msplat_num_iters) + "iter.ply"
...
func _update_msplat_terminal_header() -> void:
	...
	if msplat_dataset_root == "":
		msplat_dataset_root = export_root_path + "/splatviz_msplat_dataset_m63"
	if msplat_result_root == "":
		msplat_result_root = export_root_path + "/splatviz_msplat_result_m63"
...
func _set_msplat_dataset_folder(dir: String) -> void:
	...
	msplat_result_root = _path_parent(dataset) + "/splatviz_msplat_result_m63"
...
func _find_latest_msplat_dataset() -> void:
	...
	dir.list_dir_end()
	if best == "":
		best = export_root_path + "/splatviz_msplat_dataset_m63"
...
func _refresh_msplat_terminal(force: bool) -> void:
	if msplat_log_path == "":
		msplat_log_path = (msplat_result_root if msplat_result_root != "" else export_root_path + "/splatviz_msplat_result_m63") + "/train.log"
...
func _update_msplat_watchdog_status(full_txt: String) -> void:
	...
	if msplat_log_idle_seconds >= 240.0 and process_alive:
		...
		if not msplat_stall_notice_shown:
			...
			_append_msplat_terminal("$ WARNING: Msplat process is alive but train.log has not changed for %.0fs. Use Open Result Folder / terminal kill if CPU is 0. M6.5A keeps the UI responsive with tail-only polling." % msplat_log_idle_seconds)
...
func _export_msplat_dataset() -> void:
	...
	var export_timestamp := _m68a_timestamp()
	var dataset_root = _m68a_make_timestamped_output_root("splatviz_msplat_dataset_m63", export_timestamp)
	msplat_dataset_root = dataset_root
	DirAccess.make_dir_recursive_absolute(dataset_root + "/images")
	DirAccess.make_dir_recursive_absolute(dataset_root + "/sparse/0")
	_debug_dump_msplat_clean_render_state(dataset_root, "export_m61_before_render_loop")
	_append_msplat_terminal("$ M63 clean-render mesh dump: " + dataset_root + "/splatviz_visible_mesh_dump_m61.txt")
	for c in export_cameras:
	...
	_append_msplat_terminal("$ Sparse note: points3D.bin is robot-only synthetic seed geometry authored by SplatViz; M6.5A clean export forces performer visible and splat preview hidden.")
	if msplat_status_label:
...
func _debug_dump_msplat_camera_positions(dataset_root: String, tag: String) -> void:
	var lines: Array = []
	lines.append("SplatViz M6.7G2 camera position debug: " + tag)
...
func _ensure_msplat_dataset_current() -> void:
	...
	if DirAccess.dir_exists_absolute(msplat_dataset_root + "/images"):
		...
		_append_msplat_terminal("$ No usable images folder found at selected dataset. Exporting fresh M6.5A dataset…")
...
func _rewrite_msplat_sparse_bridge(dataset_root: String) -> void:
	...
	if pf != null:
		pf.store_string("M6.5A COLMAP images.bin policy: NAME=CAM##_frame_000001.png; Msplat resolves to sparse/0/CAM##_frame_000001.png.\n")
		pf.close()
...
func _run_msplat_smoke_test() -> void:
	...
	if int(hierarchy.get("missing", 0)) > 0:
		...
		_append_msplat_terminal("$ Fix: export a fresh M6.5A dataset. Msplat resolves image names relative to sparse/0, so the PNGs must live directly beside cameras.bin/images.bin/points3D.bin.")
		return
	msplat_result_root = export_root_path + "/splatviz_msplat_result_m63"
	DirAccess.make_dir_recursive_absolute(msplat_result_root)
	var script_path = msplat_result_root + "/run_msplat.zsh"
	var log_path = msplat_result_root + "/train.log"
	var ply_path = msplat_result_root + "/splatviz_m65a_" + _m65a_timestamp() + "_msplat_" + str(msplat_num_iters) + "iter.ply"
...
func _write_colmap_seed_audit(dataset_root: String, obs: Dictionary) -> void:
	...
	f.store_string("colmap_pose_convention=M63_world_to_camera_rows_xright_ydown_zforward_mesh_surface_rgb_seed_clean_visibility_qw_qx_qy_qz\n")
	f.close()
...
func _write_seed_points_ply_m61(dataset_root: String, points: Array) -> void:
	...
	if f == null:
		push_warning("Could not write M63 seed PLY")
		return
	f.store_string("ply\n")
...
func _write_seed_diagnostics_m61(dataset_root: String, seed_source: String, raw_count: int, kept_count: int, points: Array, obs_hist: Array, rgb_stats: Dictionary) -> void:
	...
	if f == null:
		return
	f.store_string("SplatViz M6.7G2 seed diagnostics\n")
...
func _write_nerfstudio_transforms(dataset_root: String, cams: Array, size: Vector2i) -> void:
	...
	var doc = {
		...
		"splatviz_note": "Synthetic Nerfstudio-style transforms authored by SplatViz; M6.5A uses robot-only seed points; production conclusions require gsplat validation.",
		"frames": frames
	}
...
func _m66e_add_reference_floor_and_bbox(focus_min: Vector3, focus_max: Vector3, robust_min: Vector3, robust_max: Vector3) -> void:
	# M66E1: self-contained preview reference helper.
	# Adds a lightweight floor grid below the selected cloud and a focus bbox. It does not modify PLY data.
	if splat_root == null:
		return
	var stale: Array = []
	...
	var box_pts: PackedVector3Array = PackedVector3Array([
		c000, c100, c100, c110, c110, c010, c010, c000,
		c001, c101, c101, c111, c111, c011, c011, c001,
		c000, c001, c100, c101, c110, c111, c010, c011
	])
	root.add_child(_m66e_ref_line_instance(box_pts, Color(0.38, 0.95, 1.0, 0.72), "M66E reference focus bbox"))
...
func _load_ply_point_cloud(path: String) -> int:
	...
	var pad: Vector3 = Vector3(max(0.04, robust_span.x * 0.08), max(0.04, robust_span.y * 0.08), max(0.04, robust_span.z * 0.08))
	var crop_min: Vector3 = robust_min - pad
	var crop_max: Vector3 = robust_max + pad
	var visible_records: Array = []
	for sr_any in selected_records:
		var sr: Dictionary = sr_any as Dictionary
		var sp: Vector3 = sr["p"] as Vector3
		if sp.x < crop_min.x or sp.x > crop_max.x:
			continue
		if sp.y < crop_min.y or sp.y > crop_max.y:
			continue
		if sp.z < crop_min.z or sp.z > crop_max.z:
			continue
		visible_records.append(sr)
	if visible_records.is_empty():
		visible_records = selected_records
	...
	while name != "":
		if not da.current_is_dir():
			...
			if lower.ends_with(".png") or lower.ends_with(".jpg") or lower.ends_with(".jpeg"):
				stills_images.append(dir_path.path_join(name))
		name = da.get_next()
	da.list_dir_end()
...
func _stills_sort_key(path: String) -> String:
	var cam_id: String = _stills_camera_id_from_text(path.get_file())
	if cam_id != "":
		return cam_id + "_" + path.get_file().to_lower()
	return "ZZZ_" + path.get_file().to_lower()
...
func _stills_populate_camera_option() -> void:
	...
	for i in range(stills_images.size()):
		...
		var label := ""
		if cam_id != "":
			label = cam_id + " · " + path.get_file()
		else:
			label = str(i + 1) + " · " + path.get_file()
...
func _stills_show_current() -> void:
	...
	if stills_folder_label != null:
		stills_folder_label.text = "Folder: " + stills_folder_path
...
func _stills_metadata_text(path: String, img: Image) -> String:
	...
	var text: String = "CAMERA / CAPTURE CONFIG
"
...
func _m67a1_export_camera_layout_safe() -> void:
	if has_method("_m67a_export_camera_layout"):
		...
		return
	if status_label != null:
		status_label.text = "Export Camera Layout is not installed. Re-run the M67B base package, then M67B1."
...
func _m67a1_record_snapshot_safe() -> void:
	if has_method("_m67a_record_run_snapshot"):
		...
		return
	if status_label != null:
		status_label.text = "Project history recorder is not installed. Re-run the M67B base package, then M67B1."
...
func _m67g_support_groups(data: Array) -> Dictionary:
	...
	for r_var in data:
		...
		if not groups.has(key):
			groups[key] = []
		(groups[key] as Array).append(str(r.get("id", "")))
	return groups
...
func _m67g_report_payload(report_root: String, data: Array, export_timestamp: String = "") -> Dictionary:
	...
	return {
		...
		"project_name": "SplatViz Camera Layout Blueprint Package",
...
func _m67g_report_css() -> String:
	return "body{font-family:Helvetica,Arial,sans-serif;background:#f7f6f0;color:#142031;margin:0;}main{padding:26px 30px 34px;}a{color:#194f98;text-decoration:none}a:hover{text-decoration:underline}.sheet{max-width:1320px;margin:0 auto;background:#fff;border:1px solid #c7cfdd;box-shadow:0 1px 5px rgba(20,32,49,0.08)}.titlebar{display:flex;justify-content:space-between;gap:18px;padding:18px 22px;border-bottom:2px solid #294f91;background:#fbfcff}.titleblock h1{margin:0;font-size:34px;line-height:1.05}.titleblock p{margin:6px 0 0;font-size:15px}.meta-grid{display:grid;grid-template-columns:repeat(2,minmax(160px,1fr));gap:4px 16px;font-size:13px;align-content:start}.meta-grid div{white-space:nowrap}.nav{display:flex;flex-wrap:wrap;gap:12px;padding:12px 22px;border-bottom:1px solid #d6dbe6;background:#f8fafc;font-size:13px}.content{padding:22px}.lede{font-size:15px;line-height:1.5;margin:0 0 18px}.grid-two{display:grid;grid-template-columns:1.15fr 0.85fr;gap:18px;align-items:start}.grid-three{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:14px}.card{border:1px solid #ccd4e2;background:#fcfdff;padding:14px}.card h2,.card h3{margin:0 0 10px}.card p{margin:0 0 8px;line-height:1.45}.kv{display:grid;grid-template-columns:minmax(160px,220px) 1fr;gap:6px 12px;font-size:13px}.kv div{padding:2px 0;border-bottom:1px solid #eef2f7}.kv .key{font-weight:700}.summary{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px;margin-bottom:18px}.summary .status-box{border:1px solid #cdd6e3;background:#fff;padding:12px 14px}.status-box .count{display:block;font-size:28px;font-weight:800;margin-top:4px}.pill{display:inline-block;padding:3px 8px;border-radius:999px;font-size:12px;font-weight:800}.pill.pass{background:#e7f4ea;color:#1f6b3a}.pill.warning{background:#fff3d1;color:#8a5a11}.pill.fail{background:#fee5d9;color:#9b3d1c}.pill.invalid{background:#f8d9d8;color:#9b1e1e}.note-list{margin:0;padding-left:18px;line-height:1.45}.svg-frame{border:1px solid #ccd4e2;background:#fff;padding:8px}.svg-frame img{width:100%;height:auto;display:block}.table{width:100%;border-collapse:collapse;font-size:13px}.table th,.table td{border:1px solid #ccd4e2;padding:6px 8px;text-align:left;vertical-align:top}.table th{background:#eef3fb}.small{font-size:12px;color:#4c5c71}.schedule-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:16px}.camera-card{border:1px solid #ccd4e2;background:#fff;padding:12px;break-inside:avoid;page-break-inside:avoid}.camera-card.pass{border-left:6px solid #2f7f4f}.camera-card.warning{border-left:6px solid #9d6b15}.camera-card.fail{border-left:6px solid #b24a22}.camera-card.invalid{border-left:6px solid #b32020}.camera-card img{width:100%;aspect-ratio:16/9;object-fit:contain;background:#f2f4f8;border:1px solid #d4dae5}.placeholder{width:100%;aspect-ratio:16/9;display:flex;align-items:center;justify-content:center;background:#f5ecec;border:1px solid #d7b8b8;color:#8d2525;font-weight:800;text-align:center;padding:12px;box-sizing:border-box}.camera-title{display:flex;justify-content:space-between;gap:12px;align-items:center;margin:10px 0}.camera-title h3{margin:0;font-size:20px}.footnote{font-size:12px;line-height:1.45;color:#39485b;margin-top:14px}.print-break{page-break-before:always}@media print{@page{size:landscape;margin:12mm}body{background:#fff}main{padding:0}.sheet{border:none;box-shadow:none;max-width:none}.nav{display:none}.content{padding:14mm}.schedule-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.camera-card,.card,.svg-frame{break-inside:avoid;page-break-inside:avoid}}"

func _m67g_sheet_shell(payload: Dictionary, title: String, subtitle: String, body: String) -> String:
	var nav = [
		["Overview", "sheet_01_overview.html"],
	...
	return "<!doctype html><html><head><meta charset='utf-8'><title>" + _m67g_html_escape(title) + " - SplatViz Layout Report</title><link rel='stylesheet' href='assets/report.css'></head><body><main><div class='sheet'><div class='titlebar'><div class='titleblock'><h1>" + _m67g_html_escape(title) + "</h1><p>" + _m67g_html_escape(subtitle) + "</p></div><div class='meta-grid'>" + meta_html + "</div></div><div class='nav'><a href='index.html'>Index</a>" + nav_html + "</div><div class='content'>" + body + "</div></div></main></body></html>"

func _m67g_build_index_html(payload: Dictionary) -> String:
	var counts: Dictionary = payload.get("frame_qc_counts", {})
	var body = "<p class='lede'>Use this packet to place, aim, and verify the camera array. Start with the overview sheet, then use the plan, elevation, and schedule sheets during field layout and final lockoff.</p>"
	body += "<div class='summary'>"
	body += "<div class='status-box'><b>PASS</b><span class='count'>" + str(int(counts.get("PASS", 0))) + "</span></div>"
	body += "<div class='status-box'><b>WARNING</b><span class='count'>" + str(int(counts.get("WARNING", 0))) + "</span></div>"
	body += "<div class='status-box'><b>FAIL</b><span class='count'>" + str(int(counts.get("FAIL", 0))) + "</span></div>"
	body += "<div class='status-box'><b>INVALID</b><span class='count'>" + str(int(counts.get("INVALID", 0))) + "</span></div></div>"
	body += "<div class='grid-two'><div class='card'><h2>Sheets</h2><table class='table'><tr><th>File</th><th>Use</th></tr>"
	body += "<tr><td><a href='sheet_01_overview.html'>sheet_01_overview.html</a></td><td>Project assumptions, reference dimensions, and build notes.</td></tr>"
	body += "<tr><td><a href='sheet_02_top_plan.html'>sheet_02_top_plan.html</a></td><td>Floor layout, azimuths, and support grouping.</td></tr>"
	body += "<tr><td><a href='sheet_03_front_elevation.html'>sheet_03_front_elevation.html</a></td><td>Height tiers, lens-center heights, and front-view checks.</td></tr>"
	body += "<tr><td><a href='sheet_04_side_elevation.html'>sheet_04_side_elevation.html</a></td><td>Depth checks, aim lines, and focus distance reference.</td></tr>"
	body += "<tr><td><a href='sheet_05_camera_schedule.html'>sheet_05_camera_schedule.html</a></td><td>Camera-by-camera install and verification cards.</td></tr></table></div>"
	body += "<div class='card'><h2>Files</h2><table class='table'><tr><th>File</th><th>Use</th></tr>"
	body += "<tr><td><a href='camera_mounting_schedule.csv'>camera_mounting_schedule.csv</a></td><td>Field schedule for placement, height, aim, and QC margins.</td></tr>"
	body += "<tr><td><a href='camera_layout.json'>camera_layout.json</a></td><td>Structured export of the same report data.</td></tr>"
	body += "<tr><td><a href='top_plan.svg'>top_plan.svg</a></td><td>Top plan drawing.</td></tr>"
	body += "<tr><td><a href='front_elevation.svg'>front_elevation.svg</a></td><td>Front elevation drawing.</td></tr>"
	body += "<tr><td><a href='side_elevation.svg'>side_elevation.svg</a></td><td>Side elevation drawing.</td></tr>"
	body += "<tr><td><a href='support_legend.svg'>support_legend.svg</a></td><td>Support legend and status colors.</td></tr></table></div></div>"
	return _m67g_sheet_shell(payload, "Camera Layout Blueprint Package", "Index and file map", body)
...
func _export_layout_report_to_dir(base_dir: String) -> void:
	...
	_m67h_refresh_all_camera_layout_fields()
	var export_timestamp := _m68a_timestamp()
	var report_root = chosen_dir.path_join("SplatViz_Layout_Report_" + export_timestamp)
...
func _m67g_write_report_csv_json(report_root: String, payload: Dictionary) -> void:
	...
	var csv = "camera_id,qc_status,volume_qc_status,support_type,support_id_or_zone,x_m,y_m,z_m,x_ft,y_ft,z_ft,height_m,height_ft,radius_m,radius_ft,azimuth_deg,tilt_deg,roll_deg,lens,hfov_deg,vfov_deg,focus_distance_m,focus_distance_ft,aim_target_m,focus_target_m,margin_left_pct,margin_right_pct,margin_top_pct,margin_bottom_pct,volume_margin_left_pct,volume_margin_right_pct,volume_margin_top_pct,volume_margin_bottom_pct,install_note\n"
	for r_var in data:
		var r: Dictionary = r_var as Dictionary
		var p = r["position_m"]
		...
		csv += _m67g_csv_text(str(r["id"])) + "," + _m67g_csv_text(str(r.get("subject_frame_qc_status", r.get("frame_qc_status", "")))) + "," + _m67g_csv_text(str(r.get("volume_frame_qc_status", ""))) + "," + _m67g_csv_text(str(r.get("support_type", ""))) + "," + _m67g_csv_text(str(r.get("support_id_or_zone", ""))) + ","
		csv += _m67g_num(float(p[0])) + "," + _m67g_num(float(p[1])) + "," + _m67g_num(float(p[2])) + ","
		csv += _m67g_num(float(pft[0])) + "," + _m67g_num(float(pft[1])) + "," + _m67g_num(float(pft[2])) + ","
		csv += _m67g_num(float(r["height_m"])) + "," + _m67g_num(float(r["height_ft"])) + ","
		csv += _m67g_num(float(r.get("radius_m", 0.0))) + "," + _m67g_num(float(r.get("radius_ft", 0.0))) + ","
		csv += _m67g_num(float(r["azimuth_deg"])) + "," + _m67g_num(float(r["tilt_deg"])) + "," + _m67g_num(float(r.get("roll_deg", 0.0))) + ","
		csv += _m67g_csv_text(str(r["lens"])) + "," + _m67g_num(float(r.get("hfov_deg", 0.0))) + "," + _m67g_num(float(r.get("vfov_deg", 0.0))) + ","
		csv += _m67g_num(float(r["distance_3d_m"])) + "," + _m67g_num(float(r["distance_3d_ft"])) + ","
		csv += _m67g_csv_text(_m67g_vec3_label(aim_target, 2)) + "," + _m67g_csv_text(_m67g_vec3_label(focus_target, 2)) + ","
		...
		csv += _m67g_csv_text(str(r.get("install_note", ""))) + "\n"
...
func _m67g_build_top_plan_svg(payload: Dictionary) -> String:
	...
	var cx = w * 0.5
	var cy = h * 0.50
	var svg = "<svg xmlns='http://www.w3.org/2000/svg' width='1500' height='1120' viewBox='0 0 1500 1120'>"
	svg += "<rect width='1500' height='1120' fill='#fbfbf8'/>"
	svg += "<defs><pattern id='plan_grid' width='48' height='48' patternUnits='userSpaceOnUse'><path d='M48 0H0V48' fill='none' stroke='#d8dde8' stroke-width='1'/></pattern></defs>"
	svg += "<rect width='1500' height='1120' fill='url(#plan_grid)'/>"
	...
	svg += "<rect x='" + str(stage_x) + "' y='" + str(stage_y) + "' width='" + str(stage_w) + "' height='" + str(stage_d) + "' fill='none' stroke='#244781' stroke-width='3'/>"
	svg += "<line x1='" + str(cx) + "' y1='" + str(stage_y) + "' x2='" + str(cx) + "' y2='" + str(stage_y + stage_d) + "' stroke='#5f84bf' stroke-width='1.5' stroke-dasharray='7 6'/>"
	svg += "<line x1='" + str(stage_x) + "' y1='" + str(cy) + "' x2='" + str(stage_x + stage_w) + "' y2='" + str(cy) + "' stroke='#5f84bf' stroke-width='1.5' stroke-dasharray='7 6'/>"
	svg += "<rect x='" + str(vol_x) + "' y='" + str(vol_y) + "' width='" + str(volume_size.x * scale) + "' height='" + str(volume_size.z * scale) + "' fill='rgba(92,141,222,0.10)' stroke='#3f6eb3' stroke-width='2'/>"
	svg += "<rect x='" + str(sub_x) + "' y='" + str(sub_y) + "' width='" + str(subject_size.x * scale) + "' height='" + str(subject_size.z * scale) + "' fill='rgba(53,138,84,0.10)' stroke='#2f7f4f' stroke-width='2'/>"
	svg += "<circle cx='" + str(cx) + "' cy='" + str(cy) + "' r='6' fill='#18365f'/><text x='" + str(cx + 12) + "' y='" + str(cy - 10) + "' font-size='16' font-weight='700' fill='#18365f'>Origin / capture center</text>"
	svg += "<text x='70' y='72' font-size='28' font-weight='700' fill='#17345e'>Top Plan</text>"
	svg += "<text x='70' y='102' font-size='15' fill='#2f425f'>Stage boundary, capture volume footprint, camera positions, and azimuth labels.</text>"
	for r_var in data:
		var r: Dictionary = r_var as Dictionary
		var p = r["position_m"]
		var x = cx + float(p[0]) * scale
		var y = cy - float(p[2]) * scale
		svg += "<line x1='" + str(x) + "' y1='" + str(y) + "' x2='" + str(cx) + "' y2='" + str(cy) + "' stroke='#9db2d3' stroke-width='1' stroke-dasharray='5 6'/>"
		svg += _m67g_svg_support_marker(x, y, str(r.get("support_type", "Mixed")), str(r.get("frame_qc_status", "PASS")), true)
		if str(r.get("frame_qc_status", "PASS")) == "FAIL" or str(r.get("frame_qc_status", "PASS")) == "INVALID":
			svg += "<circle cx='" + str(x) + "' cy='" + str(y) + "' r='17' fill='none' stroke='#" + _m67g_status_hex(str(r.get("frame_qc_status", ""))) + "' stroke-width='2.5'/>"
		svg += "<text x='" + str(x + 14) + "' y='" + str(y - 10) + "' font-size='14' font-weight='700' fill='#17345e'>" + _m67g_html_escape(str(r["id"])) + "</text>"
		svg += "<text x='" + str(x + 14) + "' y='" + str(y + 8) + "' font-size='11' fill='#4a5f7c'>" + _m67g_num(float(r.get("azimuth_deg", 0.0)), 0) + "°</text>"
	svg += _m67g_svg_dim_line(stage_x, stage_y + stage_d + 68.0, stage_x + stage_w, stage_y + stage_d + 68.0, _m67g_dim_label_m_ft(STAGE_W_M, 2), false)
	svg += _m67g_svg_dim_line(stage_x - 58.0, stage_y, stage_x - 58.0, stage_y + stage_d, _m67g_dim_label_m_ft(STAGE_D_M, 2), true)
	svg += _m67g_svg_dim_line(sub_x, sub_y + subject_size.z * scale + 28.0, sub_x + subject_size.x * scale, sub_y + subject_size.z * scale + 28.0, "Subject width " + _m67g_dim_label_m_ft(subject_size.x, 2), false)
	svg += _m67g_svg_dim_line(vol_x - 26.0, vol_y, vol_x - 26.0, vol_y + volume_size.z * scale, "Volume depth " + _m67g_dim_label_m_ft(volume_size.z, 2), true)
	svg += _m67g_svg_scale_bar(1120.0, 1015.0, 2.0, scale)
	svg += "</svg>"
	return svg
...
func _m67g_build_front_elevation_svg(payload: Dictionary) -> String:
	...
	var base = 730.0
	var cx = w * 0.5
	var svg = "<svg xmlns='http://www.w3.org/2000/svg' width='1500' height='880' viewBox='0 0 1500 880'>"
	svg += "<rect width='1500' height='880' fill='#fbfbf8'/>"
	svg += "<defs><pattern id='front_grid' width='52' height='52' patternUnits='userSpaceOnUse'><path d='M52 0H0V52' fill='none' stroke='#d8dde8' stroke-width='1'/></pattern></defs>"
	svg += "<rect width='1500' height='880' fill='url(#front_grid)'/>"
	svg += "<text x='70' y='72' font-size='28' font-weight='700' fill='#17345e'>Front Elevation</text>"
	svg += "<text x='70' y='102' font-size='15' fill='#2f425f'>Use this view to verify lens-center height from floor, tier spacing, and front-view aim.</text>"
	svg += "<line x1='90' y1='" + str(base) + "' x2='1410' y2='" + str(base) + "' stroke='#244781' stroke-width='3'/>"
	var volume_size = _m67g_array_to_vec3(volume.get("size_m", Vector3.ZERO))
	var volume_center = _m67g_array_to_vec3(volume.get("center_m", Vector3.ZERO))
	var subject_size = _m67g_array_to_vec3(subject.get("size_m", Vector3.ZERO))
	var subject_center = _m67g_array_to_vec3(subject.get("center_m", Vector3.ZERO))
	...
	svg += "<rect x='" + str(vol_x) + "' y='" + str(vol_y) + "' width='" + str(volume_size.x * scale_x) + "' height='" + str(volume_size.y * scale_y) + "' fill='rgba(92,141,222,0.10)' stroke='#3f6eb3' stroke-width='2'/>"
	svg += "<rect x='" + str(sub_x) + "' y='" + str(sub_y) + "' width='" + str(subject_size.x * scale_x) + "' height='" + str(subject_size.y * scale_y) + "' fill='rgba(53,138,84,0.10)' stroke='#2f7f4f' stroke-width='2'/>"
	svg += "<rect x='" + str(cx - 18.0) + "' y='" + str(base - ROBOT_HEIGHT_M * scale_y) + "' width='36' height='" + str(ROBOT_HEIGHT_M * scale_y) + "' fill='none' stroke='#5d6675' stroke-width='2'/>"
	svg += "<text x='" + str(cx + 28.0) + "' y='" + str(base - ROBOT_HEIGHT_M * scale_y + 18.0) + "' font-size='14' fill='#39485b'>Performer reference " + _m67g_dim_label_m_ft(ROBOT_HEIGHT_M, 2) + "</text>"
	...
	for tier_y in [float(tier_heights.get("low", 0.72)), float(tier_heights.get("mid", 1.55)), float(tier_heights.get("high", 2.45))]:
		...
		svg += "<line x1='90' y1='" + str(yv) + "' x2='1410' y2='" + str(yv) + "' stroke='#b3bfd2' stroke-width='1' stroke-dasharray='7 6'/>"
		svg += "<text x='96' y='" + str(yv - 8.0) + "' font-size='12' fill='#54657e'>Tier reference " + _m67g_dim_label_m_ft(tier_y, 2) + "</text>"
	for r_var in data:
		var r: Dictionary = r_var as Dictionary
		var p = r["position_m"]
		...
		svg += "<line x1='" + str(x) + "' y1='" + str(y) + "' x2='" + str(tx) + "' y2='" + str(ty) + "' stroke='#9db2d3' stroke-width='1' stroke-dasharray='5 5'/>"
		svg += _m67g_svg_support_marker(x, y, str(r.get("support_type", "Mixed")), str(r.get("frame_qc_status", "PASS")), false)
		svg += "<text x='" + str(x + 12.0) + "' y='" + str(y - 12.0) + "' font-size='13' font-weight='700' fill='#17345e'>" + _m67g_html_escape(str(r["id"])) + "</text>"
	svg += _m67g_svg_dim_line(cx - STAGE_W_M * scale_x * 0.5, base + 62.0, cx + STAGE_W_M * scale_x * 0.5, base + 62.0, _m67g_dim_label_m_ft(STAGE_W_M, 2), false)
	svg += _m67g_svg_dim_line(sub_x - 26.0, sub_y, sub_x - 26.0, sub_y + subject_size.y * scale_y, "Subject height " + _m67g_dim_label_m_ft(subject_size.y, 2), true)
	svg += _m67g_svg_dim_line(vol_x + volume_size.x * scale_x + 36.0, vol_y, vol_x + volume_size.x * scale_x + 36.0, vol_y + volume_size.y * scale_y, "Volume height " + _m67g_dim_label_m_ft(volume_size.y, 2), true)
	svg += _m67g_svg_scale_bar(1120.0, 790.0, 2.0, scale_y)
	svg += "</svg>"
	return svg
...
func _m67g_build_side_elevation_svg(payload: Dictionary) -> String:
	...
	var svg = "<svg xmlns='http://www.w3.org/2000/svg' width='1500' height='880' viewBox='0 0 1500 880'>"
	svg += "<rect width='1500' height='880' fill='#fbfbf8'/>"
	svg += "<defs><pattern id='side_grid' width='52' height='52' patternUnits='userSpaceOnUse'><path d='M52 0H0V52' fill='none' stroke='#d8dde8' stroke-width='1'/></pattern></defs>"
	svg += "<rect width='1500' height='880' fill='url(#side_grid)'/>"
	svg += "<text x='70' y='72' font-size='28' font-weight='700' fill='#17345e'>Side Elevation</text>"
	svg += "<text x='70' y='102' font-size='15' fill='#2f425f'>Use this view to verify depth from subject, focus reference, and clearance around the capture volume.</text>"
	svg += "<line x1='90' y1='" + str(base) + "' x2='1410' y2='" + str(base) + "' stroke='#244781' stroke-width='3'/>"
	var volume_size = _m67g_array_to_vec3(volume.get("size_m", Vector3.ZERO))
	var volume_center = _m67g_array_to_vec3(volume.get("center_m", Vector3.ZERO))
	var subject_size = _m67g_array_to_vec3(subject.get("size_m", Vector3.ZERO))
	...
	var sub_y = base - (subject_center.y + subject_size.y * 0.5) * scale_y
	svg += "<rect x='" + str(vol_x) + "' y='" + str(vol_y) + "' width='" + str(volume_size.z * scale_z) + "' height='" + str(volume_size.y * scale_y) + "' fill='rgba(92,141,222,0.10)' stroke='#3f6eb3' stroke-width='2'/>"
	svg += "<rect x='" + str(sub_x) + "' y='" + str(sub_y) + "' width='" + str(subject_size.z * scale_z) + "' height='" + str(subject_size.y * scale_y) + "' fill='rgba(53,138,84,0.10)' stroke='#2f7f4f' stroke-width='2'/>"
	svg += "<circle cx='" + str(cx + TARGET.z * scale_z) + "' cy='" + str(base - TARGET.y * scale_y) + "' r='7' fill='#214d93'/>"
	svg += "<text x='" + str(cx + TARGET.z * scale_z + 14.0) + "' y='" + str(base - TARGET.y * scale_y - 8.0) + "' font-size='14' fill='#214d93'>Focus target</text>"
	for r_var in data:
		var r: Dictionary = r_var as Dictionary
		var p = r["position_m"]
		...
		svg += _m67g_svg_support_marker(x, y, str(r.get("support_type", "Mixed")), str(r.get("frame_qc_status", "PASS")), false)
		svg += "<text x='" + str(x + 12.0) + "' y='" + str(y - 12.0) + "' font-size='13' font-weight='700' fill='#17345e'>" + _m67g_html_escape(str(r["id"])) + "</text>"
	svg += _m67g_svg_dim_line(cx - STAGE_D_M * scale_z * 0.5, base + 62.0, cx + STAGE_D_M * scale_z * 0.5, base + 62.0, _m67g_dim_label_m_ft(STAGE_D_M, 2), false)
	svg += _m67g_svg_dim_line(sub_x, sub_y + subject_size.y * scale_y + 28.0, sub_x + subject_size.z * scale_z, sub_y + subject_size.y * scale_y + 28.0, "Subject depth " + _m67g_dim_label_m_ft(subject_size.z, 2), false)
	svg += _m67g_svg_dim_line(vol_x, vol_y + volume_size.y * scale_y + 50.0, vol_x + volume_size.z * scale_z, vol_y + volume_size.y * scale_y + 50.0, "Volume depth " + _m67g_dim_label_m_ft(volume_size.z, 2), false)
	svg += _m67g_svg_scale_bar(1120.0, 790.0, 2.0, scale_y)
	svg += "</svg>"
	return svg
...
func _m67g_build_support_legend_svg(payload: Dictionary) -> String:
	var counts: Dictionary = payload.get("frame_qc_counts", {})
	var svg = "<svg xmlns='http://www.w3.org/2000/svg' width='980' height='260' viewBox='0 0 980 260'>"
	svg += "<rect width='980' height='260' fill='#fbfbf8' stroke='#ccd4e2'/>"
	svg += "<text x='26' y='42' font-size='24' font-weight='700' fill='#17345e'>Support and QC Legend</text>"
	var supports = ["Truss", "Stands", "Tripods", "Mixed"]
	for i in range(supports.size()):
		var name = supports[i]
		var x = 46 + i * 220
		svg += _m67g_svg_support_marker(x, 88, name, "PASS", false)
		svg += "<text x='" + str(x + 28) + "' y='94' font-size='16' fill='#17345e'>" + name + "</text>"
	var statuses = ["PASS", "WARNING", "FAIL", "INVALID"]
	for i in range(statuses.size()):
		var status = statuses[i]
		var x = 46 + i * 220
		svg += "<rect x='" + str(x - 10) + "' y='146' width='24' height='24' fill='#" + _m67g_status_hex(status) + "' opacity='0.18' stroke='#" + _m67g_status_hex(status) + "' stroke-width='2'/>"
		svg += "<text x='" + str(x + 28) + "' y='164' font-size='16' fill='#17345e'>" + status + " (" + str(int(counts.get(status, 0))) + ")</text>"
	return svg + "</svg>"
...
func _m67g_support_groups_table(payload: Dictionary) -> String:
	...
	var html = "<table class='table'><tr><th>Support type</th><th>Camera IDs</th></tr>"
...
func _m67g_camera_rows_sorted(data: Array) -> Array:
	...
	copy.sort_custom(func(a, b): return str((a as Dictionary).get("id", "")) < str((b as Dictionary).get("id", "")))
	return copy

func _m67g_build_overview_html(payload: Dictionary) -> String:
	var counts: Dictionary = payload.get("subject_frame_qc_counts", payload.get("frame_qc_counts", {}))
	var volume_counts: Dictionary = payload.get("volume_frame_qc_counts", {})
	...
	var body = "<p class='lede'>Use this packet to place, aim, and verify the camera array. Dimensions reference the stated origin unless noted, and subject training validity is evaluated separately from the wider planning volume.</p>"
	body += "<div class='summary'>"
	for status in ["PASS", "WARNING", "FAIL", "INVALID"]:
		body += "<div class='status-box'><b>" + status + "</b><span class='count'>" + str(int(counts.get(status, 0))) + "</span></div>"
	body += "</div>"
	body += "<div class='grid-two'>"
	body += "<div class='card'><h2>Setup Summary</h2><div class='kv'>"
	body += "<div class='key'>Project / stage</div><div>" + _m67g_html_escape(str(payload.get("project_name", ""))) + " / " + _m67g_html_escape(str(payload.get("stage_name", ""))) + "</div>"
	body += "<div class='key'>Export time</div><div>" + _m67g_html_escape(str(payload.get("export_timestamp", ""))) + "</div>"
	body += "<div class='key'>Array profile</div><div>" + _m67g_html_escape(str(payload.get("array_profile_name", ""))) + "</div>"
	...
	body += "<div class='key'>Capture subject bounds</div><div>" + _m67g_dim_label_m_ft(subject.x, 2) + " W, " + _m67g_dim_label_m_ft(subject.z, 2) + " D, " + _m67g_dim_label_m_ft(subject.y, 2) + " H</div>"
	body += "<div class='key'>Capture volume bounds</div><div>" + _m67g_dim_label_m_ft(volume.x, 2) + " W, " + _m67g_dim_label_m_ft(volume.z, 2) + " D, " + _m67g_dim_label_m_ft(volume.y, 2) + " H</div>"
	body += "<div class='key'>Installation mode</div><div>" + _m67g_html_escape(str(payload.get("installation_mode", ""))) + "</div>"
	body += "<div class='key'>Camera count</div><div>" + str(int(payload.get("camera_count_total", 0))) + " total, " + str(int(payload.get("camera_count_exportable", 0))) + " currently usable for clean training export</div>"
	...
	body += "<div class='key'>Lens summary</div><div>" + _m67g_html_escape(str(payload.get("lens_summary", ""))) + "</div>"
	body += "<div class='key'>Frame policy</div><div>" + _m67g_html_escape(str(payload.get("frame_policy", ""))) + "</div>"
	body += "</div></div>"
	body += "<div class='card'><h2>Practical Notes</h2><ul class='note-list'>"
	for note in payload.get("overview_notes", []):
		body += "<li>" + _m67g_html_escape(str(note)) + "</li>"
	body += "<li>Check camera IDs and mount zones against the schedule before final aim verification.</li>"
	body += "<li>Use FAIL and INVALID subject-QC cards as correction notes. Do not treat those thumbnails as approved capture frames.</li>"
	body += "<li>Volume WARNING or OUTSIDE flags are planning notes unless the subject itself is clipped.</li>"
	body += "</ul><div class='footnote'>SplatViz provides camera position and aiming references only. Final structural design, loads, overhead rigging, safety, and code compliance must be reviewed and approved by qualified production and rigging professionals.</div></div></div>"
	body += "<div class='card'><h2>QC Review</h2><p class='small'>Subject QC controls training validity. Volume QC is reported separately as a planning warning.</p><p class='small'>Volume planning warnings: PASS " + str(int(volume_counts.get("PASS", 0))) + " · WARNING " + str(int(volume_counts.get("WARNING", 0))) + " · OUTSIDE " + str(int(volume_counts.get("OUTSIDE", 0))) + "</p>" + _m67g_unsafe_camera_table(payload.get("cameras", [])) + "</div>"
	return _m67g_sheet_shell(payload, "Sheet 01 - Overview", "Project assumptions, coordinate system, and QC summary", body)

func _m67g_build_top_plan_html(payload: Dictionary) -> String:
	var body = "<p class='lede'>Use this sheet for floor marking, horizontal placement, azimuth checks, and support grouping. The inner subject footprint and the larger planning volume are both shown.</p>"
	body += "<div class='grid-two'><div class='svg-frame'><img src='top_plan.svg' alt='Top plan drawing'></div>"
	body += "<div><div class='card'><h2>Support Groups</h2>" + _m67g_support_groups_table(payload) + "</div>"
	body += "<div class='card'><h2>Legend</h2><div class='svg-frame'><img src='support_legend.svg' alt='Support legend'></div></div></div></div>"
	body += "<div class='card'><h2>Top Plan Checks</h2><ul class='note-list'><li>Mark stage center, the subject footprint, the planning volume footprint, and the X/Z centerlines first.</li><li>Verify radius and azimuth before setting final head angle.</li><li>Flag FAIL and INVALID subject-QC cameras for physical correction before any capture session.</li></ul></div>"
	body += "<div class='card'><h2>Camera Groups Requiring Attention</h2>" + _m67g_unsafe_camera_table(payload.get("cameras", [])) + "</div>"
...
func _m67g_build_front_elevation_html(payload: Dictionary) -> String:
	...
	var body = "<p class='lede'>Use this sheet to verify lens-center height from floor, tier spacing, and front-view clearance across both the subject and the planning volume.</p>"
	body += "<div class='grid-two'><div class='svg-frame'><img src='front_elevation.svg' alt='Front elevation drawing'></div>"
	body += "<div><div class='card'><h2>Height Checks</h2><table class='table'><tr><th>Tier</th><th>Reference height</th></tr>"
	...
	body += "</table></div>"
	body += "<div class='card'><h2>Verification Notes</h2><ul class='note-list'><li>Measure lens-center height from finished floor, not from support hardware.</li><li>Confirm the performer reference height, subject top, and planning-volume top before lockoff.</li><li>If a camera is too tight vertically, correct support height or location before capture.</li></ul></div></div></div>"
...
func _m67g_build_side_elevation_html(payload: Dictionary) -> String:
	...
	for r_var in _m67g_camera_rows_sorted(payload.get("cameras", [])):
		...
		if rows.size() >= 8:
			break
		rows.append("<tr><td>" + _m67g_html_escape(str(r["id"])) + "</td><td>" + _m67g_dim_label_m_ft(float(r.get("distance_3d_m", 0.0)), 2) + "</td><td>" + _m67g_html_escape(_m67g_margin_summary(r.get("subject_frame_qc_margins_pct", r.get("frame_qc_margins", {})))) + "</td></tr>")
	var body = "<p class='lede'>Use this sheet to check distance from subject, focus reference, and side-view clearance around the subject and planning volume.</p>"
	body += "<div class='grid-two'><div class='svg-frame'><img src='side_elevation.svg' alt='Side elevation drawing'></div>"
	...
	body += "</div></div>"
	body += "<div class='card'><h2>Distance Examples</h2><table class='table'><tr><th>Camera</th><th>Focus distance</th><th>Subject margins</th></tr>" + "\n".join(PackedStringArray(rows)) + "</table></div></div></div>"
	body += "<div class='card'><h2>Side-View Checks</h2><ul class='note-list'><li>Check that cable runs, stands, or tripod footprints stay clear of the performer area.</li><li>Use the aim lines as a reference for head tilt, then verify the rendered frame before final approval.</li><li>For FAIL or INVALID cameras, confirm whether the required correction is height, depth, or support position.</li></ul></div>"
...
func _m67g_camera_card_html(r: Dictionary) -> String:
	var status = str(r.get("subject_frame_qc_status", r.get("frame_qc_status", "INVALID")))
	var volume_status = str(r.get("volume_frame_qc_status", "PASS"))
	var status_class = status.to_lower()
	var volume_pill_class = "warning" if volume_status == "WARNING" else ("fail" if volume_status == "OUTSIDE" else "pass")
	var aim_target = _m67g_array_to_vec3(r.get("aim_target_m", Vector3.ZERO))
	var focus_target = _m67g_array_to_vec3(r.get("focus_target_m", Vector3.ZERO))
	var html = "<div class='camera-card " + status_class + "'><div class='camera-title'><h3>" + _m67g_html_escape(str(r["id"])) + "</h3><span class='pill " + status_class + "'>" + _m67g_html_escape(status) + "</span></div>"
	if bool(r.get("image_exists", false)):
		html += "<img src='" + _m67g_html_escape(str(r["image"])) + "' alt='" + _m67g_html_escape(str(r["id"])) + " thumbnail'>"
	else:
		html += "<div class='placeholder'>Diagnostic image not available</div>"
	html += "<div class='kv' style='margin-top:10px'>"
	html += "<div class='key'>Training QC</div><div><span class='pill " + status_class + "'>" + _m67g_html_escape(status) + "</span> " + _m67g_html_escape(str(r.get("subject_frame_qc_reason", r.get("frame_qc_reason", "")))) + "</div>"
	html += "<div class='key'>Volume QC</div><div><span class='pill " + volume_pill_class + "'>" + _m67g_html_escape(volume_status) + "</span></div>"
	html += "<div class='key'>Support</div><div>" + _m67g_html_escape(str(r.get("support_type", ""))) + " / " + _m67g_html_escape(str(r.get("support_id_or_zone", ""))) + "</div>"
	html += "<div class='key'>World position</div><div>" + _m67g_vec3_m_ft_label(_m67g_array_to_vec3(r.get("position_m", Vector3.ZERO))) + "</div>"
	html += "<div class='key'>Height from floor</div><div>" + _m67g_dim_label_m_ft(float(r.get("height_m", 0.0)), 2) + "</div>"
	html += "<div class='key'>Radius / focus distance</div><div>" + _m67g_dim_label_m_ft(float(r.get("radius_m", 0.0)), 2) + " / " + _m67g_dim_label_m_ft(float(r.get("distance_3d_m", 0.0)), 2) + "</div>"
	...
	html += "<div class='key'>Lens / FOV</div><div>" + _m67g_html_escape(str(r.get("lens", ""))) + " / HFOV " + _m67g_num(float(r.get("hfov_deg", 0.0)), 2) + "°, VFOV " + _m67g_num(float(r.get("vfov_deg", 0.0)), 2) + "°</div>"
	html += "<div class='key'>Frame policy</div><div>" + _m67g_html_escape(str(r.get("frame_policy", ""))) + "</div>"
	html += "<div class='key'>Aim target</div><div>" + _m67g_vec3_m_ft_label(aim_target) + "</div>"
	html += "<div class='key'>Focus target</div><div>" + _m67g_vec3_m_ft_label(focus_target) + "</div>"
	html += "<div class='key'>Subject margins</div><div>" + _m67g_html_escape(_m67g_margin_summary(r.get("subject_frame_qc_margins_pct", r.get("frame_qc_margins", {})))) + "</div>"
	html += "<div class='key'>Volume margins</div><div>" + _m67g_html_escape(_m67g_margin_summary(r.get("volume_frame_qc_margins_pct", {}))) + "</div>"
	html += "<div class='key'>Install note</div><div>" + _m67g_html_escape(str(r.get("install_note", ""))) + "</div>"
	html += "</div></div>"
	return html
...
func _m67g_build_schedule_html(payload: Dictionary) -> String:
	...
	var body = "<p class='lede'>Use this sheet for camera-by-camera placement, aim verification, and QC review. All cameras are listed, including subject FAIL and INVALID cameras.</p>"
	body += "<div class='card'><h2>Schedule Notes</h2><ul class='note-list'><li>PASS and WARNING subject-QC cards may use normal contact thumbnails.</li><li>FAIL and INVALID subject-QC cards are diagnostic-only and should be corrected physically before relying on them for capture.</li><li>Volume WARNING and OUTSIDE flags are planning notes and do not by themselves make a training frame invalid.</li><li>If a thumbnail is missing, use the listed position and note fields, then re-render the report after verification.</li></ul></div>"
