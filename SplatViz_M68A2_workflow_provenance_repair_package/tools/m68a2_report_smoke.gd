...
func _run() -> void:
	var report_root := OS.get_environment("M68A2_REPORT_SMOKE_ROOT")
	if report_root == "":
		report_root = "/tmp/m68a2_report_smoke"
	...
	var payload_out := OS.get_environment("M68A2_REPORT_SMOKE_JSON")
	if payload_out != "":
