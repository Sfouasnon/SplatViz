...
func _run() -> void:
	var report_root := OS.get_environment("M68A3_REPORT_SMOKE_ROOT")
	if report_root == "":
		report_root = "/tmp/m68a3_report_smoke"
	DirAccess.make_dir_recursive_absolute(report_root)
	var scene: PackedScene = load("res://scenes/Main.tscn")
	var main_node: Node = scene.instantiate()
	root.add_child(main_node)
	await process_frame
	main_node.installation_mode = "Truss"
	main_node.report_capture_specs = "36 Komodo-X array"
	main_node.report_stage_specs = "Stage 1 NOZ, Truss Build"
	main_node.report_performer_specs = "Dolly Parton performs 30 songs over 2 days"
	main_node.report_camera_label_scheme = "AA-ID 36 Camera Grid"
	main_node.report_preview_background = "Sound Stage / Wood Floor"
	main_node.report_performer_height_m = 1.83
	main_node.report_performer_height_source = "user"
	...
	var payload: Dictionary = main_node._m67g_report_payload(report_root, data, "20260528_160000")
	...
	var payload_out := OS.get_environment("M68A3_REPORT_SMOKE_JSON")
	if payload_out != "":
